...
const StickyHeader: React.FC<StickyHeaderProps> = ({
  ...
  const handleScroll = () => {
    if (headerRef.current) {
      ...
      if (headerBottom < 1 && !showStickyDiv) {
        setShowStickyDiv(true)
        setAnimateStickyDiv(true)
      } else if (headerBottom >= 1 && showStickyDiv) {
  ...
  return (
    showStickyDiv && (
      <div
        className={`${styles.stickyHeader} ${animateStickyDiv ? styles.slideIn : styles.slideOut}`}
      >
        <button className={styles.styledButton} onClick={handleButtonClick}>
          ONLINE Meeting
          <span className="ml-4 text-sm font-normal">30 ～ 40分</span>
        </button>
      </div>
    )
  )
